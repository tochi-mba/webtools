...
?>
    ...
    document.getElementById('<?php echo $project['script_id']?>').addEventListener('click', function() {
        embedModal(`<?php echo $project['active_files']?>`, `<?php echo $project['version']?>`,
            `<?php echo $project['script_id']?>`, `<?php echo $project['title']?>`,
            `<?php echo $_SESSION['uid']?>`, `<?php echo $project['libraries']?>`,
            `<?php echo $versionNumberColors[$version]?>`
            <?php echo (isset($resultParams) ? ",`".json_encode($resultParams)."`" : ",`"."[]"."`")?>,
            `<?php echo $project['automatic_variable_extraction_enabled']?>`);
    })
    </script>
...
<script>
checkbox = document.getElementById("select_<?php echo $project['script_id']?>");
checkbox.addEventListener('change', function() {
    ...
    if (document.getElementById("select_<?php echo $project['script_id']?>").checked == true) {
        selectedValue.push("<?php echo $project['script_id']?>");
    } else {
        selectedValue.splice(selectedValue.indexOf("<?php echo $project['script_id']?>"), 1);
    }
...
function show<?php echo $project['script_id']?>() {
    document.getElementById("select_<?php echo $project['script_id']?>").style.display = "block";
}

function hide<?php echo $project['script_id']?>() {
    if (document.getElementById("select_<?php echo $project['script_id']?>").checked == false) {
        document.getElementById("select_<?php echo $project['script_id']?>").style.display = "none";
    } else {
        document.getElementById("select_<?php echo $project['script_id']?>").style.display = "block";
